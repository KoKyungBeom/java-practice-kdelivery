...
public class KDeliveryMain {
  ...
  public int selectMainMenu() {
    System.out.println("[치킨의 민족 프로그램 V3]\n" +
            "-------------------------\n" +
            "1) [사장님 용] 음식점 등록하기\n" +
  ...
  //입력받은 음식점이 이미 존재하는 경우 해당 음식점에 메뉴를 확인하고 추가해야하니 해당 음식점이 존재하는지 확인하기 위한 isExistingShop 변수를 선언하고
  //shops를 순회하며 존재하는경우 true로 재할당 해준다
  public void selectAddShopMenu() {
    ...
    boolean isExistingShop = false;

    for (int i = 0; i < shops.length; i++) {
      if (shops[i] != null && shops[i].getShopName().equals(shopName)) {
        shopIdx = i;
        isExistingShop = true;
        break;
      }
  ...
  public boolean isExistOrder (int length,String customerName,String shopName,String foodName){
    for(int i=0; i < length; i++){
      if(orders[i].getCustomerName().equals(customerName) && orders[i].getShopName().equals(shopName) && orders[i].getFoodName().equals(foodName)){
        return true;
      }
    }
    return false;
  ...
  public void selectFeedbackMenu() {
    ...
    if(!isExistOrder(arrLength,customerName,shopName,foodName)){
...
//음식점,주문,리뷰등 확장성을 생각한다면 전부 배열이 아닌 리스트로 바꾸는것이 좀 더 나은 방향인것같다 음식점에 메뉴 또한 배열이 아닌 HashMap으로 바꾸고
//selectMainMenu 메서드에서 숫자가아닐때는 오류가 발생하기때문에 유효성 검증을 추가하자